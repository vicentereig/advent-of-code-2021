defmodule AdventOfCode.Octomap do
  ...
  def recharge(map) do
    map
    |>Octomap.map(fn %Octomap{x: x, y: y, energy: e} ->
      %Octomap{x: x, y: y, energy: e + 1}
    end)
  end
  ...
  def flash(map) do
    ...
    |> Enum.map(fn row ->
      row
      |> Enum.map(fn %Octomap{x: x, y: y, energy: e, flashed: flashed} = octopus ->
        if e>= 9 do
  ...
  def upper_left_flashed?(map, %Octomap{x: x, y: y}) do
    if x - 1 < 0 or y - 1 < 0, do: false, else: map |> Enum.at(x - 1) |> Enum.at(y - 1) |> to_flashed
  end
  ...
  def lower_left_flashed?(map, %Octomap{x: x, y: y}) do
    if x + 1 >= length(map) or y - 1 < 0 , do: false, else: map |> Enum.at(x + 1) |> Enum.at(y - 1) |> to_flashed
  end

  def lower_right_flashed?(map, %Octomap{x: x, y: y}) do
    if x + 1 >= length(map), do: false, else: map |> Enum.at(x + 1) |> Enum.at(y + 1)  |> to_flashed
  end
